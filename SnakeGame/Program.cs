...
internal class Program
{
    ...
    static void TimeToUpdate(object sender, ElapsedEventArgs e, ref int time, Vector2 map, List<Vector2> snake){
        time++;
    ...
    public static void RandomFruit(){
        ...
        int ramdomX = rnd.Next(0, xMap - 1);
        int ramdomY = rnd.Next(0, yMap - 1);
        fruit.x = ramdomX;
        fruit.y = ramdomY; 
    }
    private static void Main(string[] args)
    {
        ...
        //khoi tao snake moi
        List<Vector2> snake = new List<Vector2>() {new Vector2(xMap / 2, yMap / 2), new Vector2(xMap / 2 + 1, yMap / 2)};
        direction = Direction.left;
        DrawSnake(snake);
        //SnakeOutPut(snake);
        RandomFruit();
        ...
        timer.Elapsed += (sender, e) => TimeToUpdate(sender, e, ref time, map, snake);
        timer.Start();
        while(true){            
            if(!pressed && time == 1){
                GetKey();
